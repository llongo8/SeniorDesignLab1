...
| T-7-0 | 7 | Press "Send a test message" with a destination configured. | A message arrives. | **PASS** 2026-09-03 on both channels. **2026-09-10: email still reliable, the SMS gateway stopped delivering** after roughly two dozen near-identical messages in a few minutes — the app reported every one as sent with no error, so T-Mobile is filtering at `tmomail.net`. Email is our demonstrated channel; the phone field is left blank. |
