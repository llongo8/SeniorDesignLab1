...
| 5c.iv | Missing data obvious, and clearly different from off-scale data | Two distinct treatments: a gap breaks the trace, and a period with **no reading from any sensor** also greys out with a hatched band across the full plot height; an off-scale reading is clamped to the axis **and** marked with a red triangle at the edge. The grey is deliberately reserved for "nothing here at all". Shading each sensor's gap separately put grey across the plot while the *other* sensor still had readings, so its trace was drawn through the shading and looked like a line carrying on through missing data | [`app.js` `drawGapBand`](../pc-app/static/app.js) | smoke test + visual | DONE |
