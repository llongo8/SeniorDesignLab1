...
| 5c.iv | Missing data obvious, and clearly different from off-scale data | Two distinct treatments: a gap breaks the trace **and** fills that sensor's own **S1/S2 lane under the plot**, in its colour, at the same x; an off-scale reading is clamped to the axis **and** marked with a red triangle at the edge. Missing data was originally shaded *across* the plot, which put sensor 1's shading straight through sensor 2's trace whenever sensor 2 read in that part of the scale — the trace was correct and broke properly, but it looked like a sensor was missing while its line carried on. A lane cannot be crossed by a trace, so the two readings of the chart always agree | [`app.js` `drawGapBand`](../pc-app/static/app.js) | smoke test + visual | DONE |
